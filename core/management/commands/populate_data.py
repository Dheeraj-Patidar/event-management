    ...
    def handle(self, *args, **options):
        ...
        for i in range(1, 6):
            ...
            if not User.objects.filter(username=username).exists():
                ...
                self.stdout.write(
                    self.style.WARNING(f"⚠️ {username} already exists")
                )
        self.stdout.write(
            self.style.SUCCESS(f"✅ {total_user_created} Users created")
        )
        ...
        #     else:
        #         self.stdout.write(
        #             self.style.WARNING(f"⚠️ event{i} Event already exists")
        #         )
        # self.stdout.write(
        #     self.style.SUCCESS(f"✅ {total_event_created} Events created")
        # )
